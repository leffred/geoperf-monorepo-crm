// GEOPERF Edge Function — Rendu HTML d'un livre blanc + upload Storage
// Trigger: POST {report_id, sections} (sections = JSON output de la synthèse Sonnet/Opus côté n8n)
// Output: {ok, html_url, html_size}
//
// Pipeline :
//   1. SELECT companies depuis DB (report_id)
//   2. Build chart data
//   3. Render HTML (template editorial inline)
//   4. Upload Storage white-papers/<report_id>.html
//   5. Create signed URL (7 jours)
//   6. UPDATE reports.html_url
//   7. Return {ok, html_url}
...
function computeGeoDistribution(companies: Company[]) {
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    const a2 = (cumulative + fraction) * 2 * Math.PI - Math.PI / 2;
...
function computeVisibilityPyramid(stats: any) {
  ...
  return layerData.map(([label, count, color], i) => {
    ...
    const width = Math.max(120, Math.round(80 + ratio * 480));
    const x = 20 + (560 - width) / 2;
    const y = 20 + i * 48;
...
function computeLLMBars(companies: Company[], providers: string[]) {
  ...
  for (const c of companies) for (const p of providers) if (c.cited_by?.[p]) counts[p]++;
  const pretty: Record<string, [string, string]> = {
    perplexity: ["Perplexity", "Sonar Pro"], openai: ["GPT-4o", "OpenAI"],
    google: ["Gemini 2.5", "Google"], anthropic: ["Sonnet 4.6", "Anthropic"],
  };
...
function renderHTML(report: any, sections: any, stats: any, companies: Company[], charts: any): string {
...
h2, h3 { page-break-after: avoid; }`;
  ...
  // Exec
  html += `<section class="page-break"><div class="caps">Section 01</div><h2>Résumé exécutif</h2><div class="exec-block">${escape(sections.executive_summary)}</div><h3>Chiffres clés</h3><div class="kpi-grid"><div class="kpi-card featured"><div class="num">${report.companies_count}</div><div class="lbl">Sociétés identifiées</div></div><div class="kpi-card"><div class="num">${(stats.cited_by_4_llms||0)+(stats.cited_by_3_llms||0)}</div><div class="lbl">Top tier (3-4 LLM)</div></div><div class="kpi-card"><div class="num">${report.llms_count}</div><div class="lbl">LLM interrogés</div></div><div class="kpi-card"><div class="num">${report.top_n}</div><div class="lbl">Position max</div></div></div></section>`;
  ...
  html += `<section class="page-break"><div class="caps">Section 02</div><h2>Méthodologie</h2><p>${escape(sections.methodology)}</p><h3>Modèles LLM interrogés</h3><div class="kpi-grid">${llmCards}</div><h3>Pipeline</h3><ol style="margin-left:6mm;"><li><strong>Extraction parallèle</strong> : prompt structuré JSON.</li><li><strong>Consolidation</strong> : dédoublonnage par domaine + nom normalisé.</li><li><strong>Scoring visibilité</strong> 0-${report.llms_count}, position moyenne, sources distinctes.</li><li><strong>Stockage Supabase</strong> pour audit complet.</li><li><strong>Synthèse</strong> automatisée par LLM (Sonnet 4.6).</li></ol><h3>Limites</h3><p class="subtle">Étude à un instant donné. Mise à jour semestrielle. Les sociétés non citées peuvent dominer le marché mais souffrir d'une moindre indexation web.</p></section>`;
  ...
  const insights = (sections.insights_and_recommendations || []).map((i: any) =>
    `<div class="insight"><div class="title">${escape(i.title)}</div><div>${escape(i.body)}</div></div>`
  ).join("");
  html += `<section class="page-break"><div class="caps">Section 06</div><h2>Insights &amp; recommandations</h2>${insights}</section>`;

  // Glossary + FAQ
  html += `<section class="page-break"><div class="caps">Section 07 — Annexe</div><h2>Glossaire</h2>
    <div class="glossary-item"><div class="term">LLM</div><div>Modèle d'IA entraîné sur de vastes corpus. ChatGPT, Gemini, Claude, Perplexity.</div></div>
    <div class="glossary-item"><div class="term">GEO (Generative Engine Optimization)</div><div>Discipline marketing visant à optimiser la visibilité dans les LLM. Successeur du SEO.</div></div>
    <div class="glossary-item"><div class="term">Score de visibilité IA</div><div>0-${report.llms_count} : nombre de LLM ayant cité la société.</div></div>
    <div class="glossary-item"><div class="term">Cohérence inter-LLM</div><div>Stabilité du classement à travers les LLM.</div></div>
    <h2 style="margin-top:14mm;">FAQ</h2>
    <div class="faq-item"><div class="q">Pourquoi 4 LLM ?</div><div class="a">Pour identifier les sociétés universellement reconnues vs perceptions singulières.</div></div>
    <div class="faq-item"><div class="q">Comment améliorer mon score ?</div><div class="a">Bloomberg/FT/Reuters, Wikipédia, Forrester/Gartner. Geoperf propose des audits.</div></div>
    <div class="faq-item"><div class="q">Mise à jour ?</div><div class="a">Édition semestrielle.</div></div>
    <div class="faq-item"><div class="q">Étude sur ma catégorie ?</div><div class="a">Sur mesure. 7 jours. contact@geoperf.com.</div></div>
  </section>`;

  // About
  html += `<section class="page-break"><div class="caps">Section 08</div><h2>À propos de Geoperf</h2><div class="about-box"><h3>Notre mission</h3><p>${escape(sections.about_geoperf)}</p></div><h3>Contact</h3><p><strong>Site :</strong> geoperf.com<br><strong>Email :</strong> contact@geoperf.com<br><strong>LinkedIn :</strong> linkedin.com/company/geoperf</p><h3>Audits sur mesure</h3><p>Diagnostic GEO complet, comparaison concurrentielle, plan d'action. Réservez sur geoperf.com/contact.</p><div class="legal-footer">Geoperf est un produit de Jourdechance SAS · SIREN 838 114 619 · RCS Nanterre · 31 rue Diaz, 92100 Boulogne-Billancourt<br>Étude générée le ${escape(report.generated_at_human)} · ID : ${escape(report.report_id)}<br>Reproduction interdite. Données issues des LLM interrogés.</div></section>`;
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!sections) return new Response(JSON.stringify({ error: "sections required" }), { status: 400 });
    ...
    const { data: rcData, error: rcErr } = await supabase
      ...
      .order("rank");
    if (rcErr) throw new Error(`DB: ${rcErr.message}`);
    if (!rcData || rcData.length === 0) throw new Error(`No companies for ${reportId}`);
    ...
    const charts = {
      ...
      llm_generosity: computeLLMBars(companies, providers),
    };

    const llmPretty: Record<string, any> = {
    ...
    const now = new Date();
    const dateFmt = (d: Date) => d.toLocaleDateString("fr-FR", { day: "numeric", month: "long", year: "numeric" });
    const monthYear = (d: Date) => d.toLocaleDateString("fr-FR", { month: "long", year: "numeric" });

    const report = {
      title: reportRow?.sous_categorie || "Étude sectorielle",
      subtitle: "État de la visibilité des acteurs majeurs dans les LLM en 2026.",
      serie_label: "LLM Visibility Research",
      period: monthYear(now),
      edition_label: monthYear(now),
      generated_at_human: dateFmt(now),
      report_id: reportId,
      report_id_short: reportId.substring(0, 8).toUpperCase(),
      llms_count: providers.length,
      llm_list: providers.map(p => llmPretty[p]),
      companies_count: stats.total_unique_companies,
      top_n: Math.min(topN, sections.top_companies_summary?.length || companies.length),
    };

    const html = renderHTML(report, sections, stats, companies, charts);

    // Upload Storage
    const fileName = `${reportId}.html`;
    const { error: upErr } = await supabase.storage
      .from("white-papers")
      .upload(fileName, new Blob([html], { type: "text/html; charset=utf-8" }), { upsert: true });
    ...
    if (urlErr) throw new Error(`Signed URL: ${urlErr.message}`);
    const htmlUrl = urlData.signedUrl;

    // ─────────────────────────────────────────────
    // PDF rendering via PDFShift (best-effort, ne bloque pas si KO)
    // ─────────────────────────────────────────────
    ...
    if (pdfshiftKey) {
      try {
        ...
        if (!pdfResp.ok) {
          throw new Error(`PDFShift HTTP ${pdfResp.status}: ${(await pdfResp.text()).slice(0, 300)}`);
        }
        ...
        const { error: pdfUpErr } = await supabase.storage
          .from("white-papers")
          .upload(pdfFileName, new Blob([pdfBytes], { type: "application/pdf" }), { upsert: true });
        if (pdfUpErr) throw new Error(`Storage PDF: ${pdfUpErr.message}`);
        const { data: pdfUrlData, error: pdfUrlErr } = await supabase.storage
          .from("white-papers")
          .createSignedUrl(pdfFileName, 60 * 60 * 24 * 7);
        if (pdfUrlErr) throw new Error(`Signed URL PDF: ${pdfUrlErr.message}`);
        pdfUrl = pdfUrlData.signedUrl;
      } catch (pdfE: any) {
        console.error("[PDF ERROR]", pdfE);
        pdfError = pdfE.message;
      }
    } else {
      pdfError = "PDFSHIFT_API_KEY not set — PDF skipped";
    }
